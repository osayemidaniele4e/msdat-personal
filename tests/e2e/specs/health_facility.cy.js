describe('Health Facility Page', () => {
  beforeEach(() => {
    cy.visit('https://msdat.fmohconnect.gov.ng/dashboard/Health_Facility');
  });

  it('should contain Health_Facility in the URL', () => {
    cy.url().should('include', 'Health_Facility');
  });
...
describe('Health Facility Config API', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboards/6/',
  ...
  it('should have the correct name', () => {
    expect(responseBody).to.have.property('name', 'Health_Facility');
  });
...
describe('Dashboard Indicators', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboard-data/indicators/6/',
...
describe('Dashboard Data', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboard-data/6/?caching=true',
      timeout: 180000, // wait up to 2 minutes
    }).then((response) => {
      expect(response.status).to.eq(200);
      responseBody = response.body;
