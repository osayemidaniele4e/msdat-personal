describe('Demographic Page', () => {
  beforeEach(() => {
    cy.visit('https://msdat.fmohconnect.gov.ng/dashboard/Demographics');
  });

  it('should contain Demographics in the URL', () => {
    cy.url().should('include', 'Demographics');
  });
...
describe('Demographics Config API', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboards/3/',
  ...
  it('should have the correct name', () => {
    expect(responseBody).to.have.property('name', 'Demographics');
  });
...
describe('Dashboard Indicators', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboard-data/indicators/3/',
...
describe('Dashboard Data', () => {
  ...
  before(() => {
    ...
    cy.request({
      method: 'GET',
      url: 'https://msdat-api.fmohconnect.gov.ng/api/dashboard-data/3/?caching=true',
      timeout: 300000, // wait up to 2 minutes
    }).then((response) => {
      expect(response.status).to.eq(200);
      responseBody = response.body;
    });
